	object_const_def
	const_export FIGHTINGDOJO_KARATE_MASTER
	const_export FIGHTINGDOJO_BLACKBELT1
	const_export FIGHTINGDOJO_BLACKBELT2
	const_export FIGHTINGDOJO_BLACKBELT3
	const_export FIGHTINGDOJO_BLACKBELT4
	const_export FIGHTINGDOJO_HITMONLEE_POKE_BALL
	const_export FIGHTINGDOJO_HITMONCHAN_POKE_BALL

FightingDojo_Object:
	db $3 ; border block

	def_warp_events
	warp_event  4, 11, LAST_MAP, 2
	warp_event  5, 11, LAST_MAP, 2

	def_bg_events

	def_object_events
	object_event  5,  3, SPRITE_BLACKBELT, STAY, DOWN, TEXT_FIGHTINGDOJO_KARATE_MASTER, OPP_COMRADE, 9
	object_event  3,  4, SPRITE_YOUNGSTER, STAY, RIGHT, TEXT_FIGHTINGDOJO_BLACKBELT1, OPP_COMRADE, 10
	object_event  3,  6, SPRITE_YOUNGSTER, STAY, RIGHT, TEXT_FIGHTINGDOJO_BLACKBELT2, OPP_COMRADE, 11
	object_event  5,  5, SPRITE_YOUNGSTER, STAY, LEFT, TEXT_FIGHTINGDOJO_BLACKBELT3, OPP_COMRADE, 12
	object_event  5,  7, SPRITE_YOUNGSTER, STAY, LEFT, TEXT_FIGHTINGDOJO_BLACKBELT4, OPP_COMRADE, 13
	object_event  4,  1, SPRITE_POKE_BALL, STAY, NONE, TEXT_FIGHTINGDOJO_HITMONLEE_POKE_BALL
	object_event  5,  1, SPRITE_POKE_BALL, STAY, NONE, TEXT_FIGHTINGDOJO_HITMONCHAN_POKE_BALL

	def_warps_to FIGHTING_DOJO
